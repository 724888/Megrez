...
uint8* WriteFloatVarint(float  value) {
	return WriteUInt32Varint(EncodeFloat(value));
}

uint8* WriteDoubleVarint(double value) {
	return WriteUInt64Varint(EncodeDouble(value));
}

float ReadFloatVarint(uint8* value) {
	return DecodeFloat(ReadUInt32Varint(value));
}

double ReadDoubleVarint(uint8* value) {
	return DecodeDouble(ReadUInt64Varint(value));
}
